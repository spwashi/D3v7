...
(function (global, factory) {
    ...
    function cubehelixConvert(o) {
        ...
        if (!(o instanceof Rgb))
            o = rgbConvert(o);
        var r = o.r / 255, g = o.g / 255, b = o.b / 255, l = (BC_DA * b + ED * r - EB * g) / (BC_DA + ED - EB), bl = b - l, k = (E * (g - l) - C * bl) / D$1, s = Math.sqrt(k * k + bl * bl) / (E * l * (1 - l)), // NaN if l=0 or l=1
        ...
        function started(event) {
            ...
            function move(event) {
                ...
                switch (mode) {
                    ...
                    case MODE_DRAG: {
                        if (signX)
                            dx = max$2(W - w0, min$1(E - e0, dx)), w1 = w0 + dx, e1 = e0 + dx;
                        if (signY)
                    ...
                    case MODE_HANDLE: {
                        if (points[1]) {
                            if (signX)
                                w1 = max$2(W, min$1(E, points[0][0])), e1 = max$2(W, min$1(E, points[1][0])), signX = 1;
                            if (signY)
                                n1 = max$2(N, min$1(S, points[0][1])), s1 = max$2(N, min$1(S, points[1][1])), signY = 1;
                        }
                        else {
                            if (signX < 0)
                                dx = max$2(W - w0, min$1(E - w0, dx)), w1 = w0 + dx, e1 = e0;
                            else if (signX > 0)
                                dx = max$2(W - e0, min$1(E - e0, dx)), w1 = w0, e1 = e0 + dx;
                            if (signY < 0)
                    ...
                    case MODE_CENTER: {
                        if (signX)
                            w1 = max$2(W, min$1(E, w0 - dx * signX)), e1 = max$2(W, min$1(E, e0 + dx * signX));
                        if (signY)
    ...
    function density() {
        var x = defaultX$1, y = defaultY$1, weight = defaultWeight, dx = 960, dy = 500, r = 20, // blur radius
        ...
        _step(i, x, y) {
            ...
            do {
                ...
                if (e === -1) {
                    ...
                    if (e !== t) {
                        if (pow$2(x - points[e * 2], 2) + pow$2(y - points[e * 2 + 1], 2) < dc)
                            return e;
                    }
                    break;
                }
            ...
            function apply(quad, x0, y0, x1, y1) {
                ...
                if (data) {
                    if (data.index > node.index) {
                        ...
                        if (l < r * r) {
                            if (x === 0)
                                x = jiggle(random), l += x * x;
                            if (y === 0)
                                y = jiggle(random), l += y * y;
    ...
    function centroidRingPoint(lambda, phi) {
        lambda *= radians, phi *= radians;
        var cosPhi = cos$1(phi), x = cosPhi * cos$1(lambda), y = cosPhi * sin$1(lambda), z = sin$1(phi), cx = y0$4 * z - z0 * y, cy = z0 * x - x0$4 * z, cz = x0$4 * y - y0$4 * x, m = hypot(cx, cy, cz), w = asin$1(m), // line weight = angle
    ...
    function circle$2() {
        var center = constant$3([0, 0]), radius = constant$3(90), precision = constant$3(6), ring, rotate, stream = { point: point };
        function point(x, y) {
    ...
    var clipAntimeridian = clip(function () { return true; }, clipAntimeridianLine, clipAntimeridianInterpolate, [-pi$1, -halfPi$1]);
            ...
            point: function (lambda1, phi1) {
                ...
                if (abs$1(delta - pi$1) < epsilon$1) { // line crosses a pole
                    ...
                    if (abs$1(lambda0 - sign0) < epsilon$1)
                        lambda0 -= sign0 * epsilon$1; // handle degeneracies
    ...
    function clipCircle(radius) {
        var cr = cos$1(radius), delta = 6 * radians, smallRadius = cr > 0, notHemisphere = abs$1(cr) > epsilon$1; // TODO optimise for this common case
    ...
    function conicConformalRaw(y0, y1) {
        ...
        function project(x, y) {
            if (f > 0) {
                if (y < -halfPi$1 + epsilon$1)
                    y = -halfPi$1 + epsilon$1;
            }
            else {
                if (y > halfPi$1 - epsilon$1)
                    y = halfPi$1 - epsilon$1;
    ...
    function identity$4() {
        ...
        kx = 1, ky = 1, transform = transformer$3({
            point: function (x, y) {
    ...
    function line(x, y$1) {
        ...
        function line(data) {
            ...
            for (i = 0; i <= n; ++i) {
                ...
                if (defined0)
                    output.point(+x(d, i, data), +y$1(d, i, data));
            }
            if (buffer)
    ...
    function area(x0, y0, y1) {
        var x1 = null, defined = constant$1(true), context = null, curve = curveLinear, output = null;
